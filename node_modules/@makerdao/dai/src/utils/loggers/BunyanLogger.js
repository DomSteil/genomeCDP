'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = undefined;

var _getPrototypeOf = require('babel-runtime/core-js/object/get-prototype-of');

var _getPrototypeOf2 = _interopRequireDefault(_getPrototypeOf);

var _classCallCheck2 = require('babel-runtime/helpers/classCallCheck');

var _classCallCheck3 = _interopRequireDefault(_classCallCheck2);

var _createClass2 = require('babel-runtime/helpers/createClass');

var _createClass3 = _interopRequireDefault(_createClass2);

var _possibleConstructorReturn2 = require('babel-runtime/helpers/possibleConstructorReturn');

var _possibleConstructorReturn3 = _interopRequireDefault(_possibleConstructorReturn2);

var _inherits2 = require('babel-runtime/helpers/inherits');

var _inherits3 = _interopRequireDefault(_inherits2);

var _typeof2 = require('babel-runtime/helpers/typeof');

var _typeof3 = _interopRequireDefault(_typeof2);

var _ServiceManager = require('../../core/ServiceManager');

var _ServiceManager2 = _interopRequireDefault(_ServiceManager);

var _PrivateService2 = require('../../core/PrivateService');

var _PrivateService3 = _interopRequireDefault(_PrivateService2);

var _bunyan = require('bunyan');

var _bunyan2 = _interopRequireDefault(_bunyan);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

/**
 * @returns {string}
 * @private
 */
function _guid() {
  function s4() {
    return Math.floor((1 + Math.random()) * 0x10000).toString(16).substring(1);
  }
  return s4() + s4() + '-' + s4() + '-' + s4() + '-' + s4() + '-' + s4() + s4() + s4();
}

function _appendState(service) {
  return function (args) {
    if ((0, _typeof3.default)(args[0]) === 'object') {
      args[0].state = service.manager().state();
    } else {
      args = [{ state: service.manager().state() }].concat(args);
    }

    return args;
  };
}

var BunyanLogger = function (_PrivateService) {
  (0, _inherits3.default)(BunyanLogger, _PrivateService);

  /**
   * @param {string} name
   */
  function BunyanLogger() {
    var name = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'log';
    (0, _classCallCheck3.default)(this, BunyanLogger);

    var _this = (0, _possibleConstructorReturn3.default)(this, (BunyanLogger.__proto__ || (0, _getPrototypeOf2.default)(BunyanLogger)).call(this, name));

    _this._logger = _bunyan2.default.createLogger({
      name: 'makerdao',
      level: 'debug',
      serializers: { err: _bunyan2.default.stdSerializers.err },
      client: _guid()
    });
    return _this;
  }

  (0, _createClass3.default)(BunyanLogger, [{
    key: 'serviceLogger',
    value: function serviceLogger(service) {
      var name = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

      if (!_ServiceManager2.default.isValidService(service)) {
        throw new Error('Invalid service object');
      }

      var log = this._logger.child({ svc: name || service.manager().name() }),
          append = _appendState(service),
          wrapper = {
        trace: function trace() {
          for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
            args[_key] = arguments[_key];
          }

          return log.trace.apply(log, append(args));
        },
        debug: function debug() {
          for (var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
            args[_key2] = arguments[_key2];
          }

          return log.debug.apply(log, append(args));
        },
        info: function info() {
          for (var _len3 = arguments.length, args = Array(_len3), _key3 = 0; _key3 < _len3; _key3++) {
            args[_key3] = arguments[_key3];
          }

          return log.info.apply(log, append(args));
        },
        warn: function warn() {
          for (var _len4 = arguments.length, args = Array(_len4), _key4 = 0; _key4 < _len4; _key4++) {
            args[_key4] = arguments[_key4];
          }

          return log.warn.apply(log, append(args));
        },
        error: function error() {
          for (var _len5 = arguments.length, args = Array(_len5), _key5 = 0; _key5 < _len5; _key5++) {
            args[_key5] = arguments[_key5];
          }

          return log.error.apply(log, append(args));
        },
        fatal: function fatal() {
          for (var _len6 = arguments.length, args = Array(_len6), _key6 = 0; _key6 < _len6; _key6++) {
            args[_key6] = arguments[_key6];
          }

          return log.fatal.apply(log, append(args));
        }
      };

      service.manager().onStateChanged(function (from, to) {
        wrapper.debug({ transition: { from: from, to: to } }, 'State changed.');
      });

      return wrapper;
    }
  }, {
    key: 'trace',
    value: function trace() {
      for (var _len7 = arguments.length, args = Array(_len7), _key7 = 0; _key7 < _len7; _key7++) {
        args[_key7] = arguments[_key7];
      }

      this._logger.trace.apply(this._logger, args);
    }
  }, {
    key: 'debug',
    value: function debug() {
      for (var _len8 = arguments.length, args = Array(_len8), _key8 = 0; _key8 < _len8; _key8++) {
        args[_key8] = arguments[_key8];
      }

      this._logger.debug.apply(this._logger, args);
    }
  }, {
    key: 'info',
    value: function info() {
      for (var _len9 = arguments.length, args = Array(_len9), _key9 = 0; _key9 < _len9; _key9++) {
        args[_key9] = arguments[_key9];
      }

      this._logger.info.apply(this._logger, args);
    }
  }, {
    key: 'warn',
    value: function warn() {
      for (var _len10 = arguments.length, args = Array(_len10), _key10 = 0; _key10 < _len10; _key10++) {
        args[_key10] = arguments[_key10];
      }

      this._logger.warn.apply(this._logger, args);
    }
  }, {
    key: 'error',
    value: function error() {
      for (var _len11 = arguments.length, args = Array(_len11), _key11 = 0; _key11 < _len11; _key11++) {
        args[_key11] = arguments[_key11];
      }

      this._logger.error.apply(this._logger, args);
    }
  }, {
    key: 'fatal',
    value: function fatal() {
      for (var _len12 = arguments.length, args = Array(_len12), _key12 = 0; _key12 < _len12; _key12++) {
        args[_key12] = arguments[_key12];
      }

      this._logger.fatal.apply(this._logger, args);
    }
  }, {
    key: 'connect',
    value: function connect() {}
  }, {
    key: 'authenticate',
    value: function authenticate() {}
  }]);
  return BunyanLogger;
}(_PrivateService3.default);

exports.default = BunyanLogger;